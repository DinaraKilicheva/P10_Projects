...
from keybords import languages_inline_btn, share_phone_btn, confirmation_inline_btn
from messages import messages
from states import StudentRegistrationForm
from task import Chat
from utils import write_row_to_csv, get_fullname
...
@bot.callback_query_handler(lambda call: call.data.startswith("action_language_"))
# def set_language_query_handler(call):
#     message = call.message
#     lang_code = call.data.split("_")[1]
#     chat = message.chat
...
def course_get(message):
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        msg += "Ma'lumotlarni tasdiqlang"
        bot.send_message(message.chat.id, msg, reply_markup=confirmation_inline_btn)
    bot.set_state(message.from_user.id, StudentRegistrationForm.confirm, message.chat.id)


@bot.callback_query_handler(lambda call: call.data.startswith("confirmation_"),
                            state=StudentRegistrationForm.first_name)
def set_language_query_handler(call):
    message = call.message
    ...
    if conf == "yes":
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            write_row_to_csv("students.csv", list(student.keys()), student)
            bot.send_message(message.chat.id, "Registratsiyangiz muvaffaqiyatli tugadi!")

    else:
        bot.send_message(message.chat.id, "Iltimos teppadagi tugmachalrdan birini tanlang!")
